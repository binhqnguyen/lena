 ...
 */
NS_LOG_COMPONENT_DEFINE ("rlcam-experiment");
int
...
CreateUes(1,5.0,1000);	//current UEs constantly in the cell.
...
get_average_result(){
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter){
    ns3::Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow(iter->first);
		*throughput_goodput_delay_wp->GetStream() << 8*iter->second.rxBytes/(iter->second.timeLastRxPacket-iter->second.timeFirstRxPacket)*ONEBIL/(1024) << "\t" << 8*iter->second.txBytes/(iter->second.timeLastTxPacket-iter->second.timeFirstTxPacket)*ONEBIL/(1024) << "\t" << iter->second.delaySum/iter->second.rxPackets/1000000 << std::endl;
...
CreateUes(uint32_t number_of_ues, double startTime, double on_time){
  ...
  *debugger_wp->GetStream() << "--------------\nUE attachs: "
            ...
            << "\n----------------\n";
  for (uint16_t i = 0; i < number_of_ues; i++)
    {
  ...
  for (uint32_t u = 0; u < ueNodes.GetN(); ++u){
    ...
    *debugger_wp->GetStream() << "\nUE IP " << ue->GetObject<Ipv4>() 
    ...
    for (uint32_t b = 0; b < number_of_bearers_per_ue; ++b){
      ...
      if (isTcp == 1){
        ...
        //onOffHelper.SetAttribute("MaxBytes", UintegerValue(1000000)); //send 1MB file.
    ...
    *debugger_wp->GetStream() << "UE-ID # " << u
      ...
      << " TCP = " << isTcp << std::endl;
		...
		//lteHelper->HandoverRequest (Seconds (5.00), ueLteDevs.Get (0), enbLteDevs.Get (0), enbLteDevs.Get (1));
    //lteHelper->HandoverRequest (Seconds (20.00), ueLteDevs.Get (0), enbLteDevs.Get (1), enbLteDevs.Get (0));
...
void InstallMobilityEnb(NodeContainer enbNodes, double distance_among_enbs_x, double distance_among_enbs_y){
  ...
  *debugger_wp->GetStream() << "\n--------------\nENBs allocation:\n-----------\n"
  ...
  for (uint16_t i = 0; i < numberOfEnbs; i++)
    {
      Vector enbPosition (distance_among_enbs_x * i, 0, 0);
...
void InstallMobilityUe(NodeContainer ueNodes, uint16_t is_pedestrian){
  ...
  *debugger_wp->GetStream() << "\n-------------\nUEs Mobility:\n------------\n";
	...
	if (!is_pedestrian){
 		//vehicular speed is 40km/h mean, variance 5km/h.
		normal_db->SetAttribute("Mean", DoubleValue(VEH_VE));
		normal_db->SetAttribute("Variance", DoubleValue(5));
		for (int i = 0; i < seconds % 100; i++){
			moving_speed = normal_db->GetValue();     //vehicular mobility
		}
	}
	else{
		//pedestrian speed is 3km/h mean, variance 0.5km/h.
		normal_db->SetAttribute("Mean", DoubleValue(PED_VE));
		normal_db->SetAttribute("Variance", DoubleValue(0.5));
		for (int i = 0; i < seconds % 100; i++){
			moving_speed = normal_db->GetValue();     //pedestrian mobility
		}
	}
	double speed_mps = double (moving_speed)*1000/3600 + 1; //kmph to meter per second.
	...
	ueMobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
			...
			"Time", StringValue ("20s"), //change direction and speed after each 2s.
			"Speed", StringValue ("ns3::NormalRandomVariable[Mean="+ms+"|Variance=1.0|Bound=5.0]"),  //m/s
			//"Speed", StringValue ("ns3::NormalRandomVariable[Mean=3]"),  //m/s
			"Bounds", RectangleValue (Rectangle (-moving_bound, moving_bound, -moving_bound, moving_bound)));  //bound

   	normal_db->SetAttribute("Mean", DoubleValue(distance_rho));
    normal_db->SetAttribute("Variance", DoubleValue(20));
		double rho = 0;
    for (int i = 0; i < seconds % 100; i++){
       rho = normal_db->GetValue();    
    }
    ...
    ueMobility.SetPositionAllocator ("ns3::UniformDiscPositionAllocator", //nodes are put randomly inside a circle with the central point is (x,y).
    	"X", DoubleValue (0),
    	"Y", DoubleValue (0),
    	"rho", DoubleValue (rho));  //radius of the circle.
		ueMobility.Install(ueNodes);
    *debugger_wp->GetStream() << "Random position allocation: circle center = <0,0>. "
        << "circle radius = " << rho << std::endl;
		*/
	//UE moves along x-axis
	/*
	ueMobility.SetMobilityModel ("ns3::ConstantVelocityMobilityModel");
	ueMobility.Install(ueNodes);
	for (uint16_t i = 0; i < ueNodes.GetN(); i++){
		ueNodes.Get (i)->GetObject<MobilityModel> ()->SetPosition (Vector (distance_ue_root_x, distance_ue_root_y, 0));
		*debugger_wp->GetStream() << "UE " << i << " speed = " << speed_mps; 
  	ueNodes.Get (i)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed_mps, 0, 0));
	}
  ...
  ueMobility.SetPositionAllocator ("ns3::GridPositionAllocator",
        "MinX", DoubleValue (rho),  //1st UE is put at a location which is "distance" meters away from eNB.
        "MinY", DoubleValue (rho),
...
init_wrappers(){
  ...
  *position_tracking_wp->GetStream() << "========================\n";
  ...
  if (isTcp==1){
	...
	throughput_goodput_delay_wp = asciiTraceHelper.CreateFileStream(throughput_goodput_delay, std::ios::app);
