    ...
    for (uint32_t b = 0; b < number_of_bearers_per_ue; ++b){
      ...
      if (isTcp == 1){
        ...
        OnOffHelper onOffHelper("ns3::TcpSocketFactory", Address ( InetSocketAddress(ueIpIfaces.GetAddress(u) , dlPort) ));
				...
				onOffHelper.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=5.0]"));
				onOffHelper.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=5.0]"));
