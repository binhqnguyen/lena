...
public:
	...
	for (size_t i = 0;; i++) {
		int ret = sysctl_getnum(i, name, sizeof(name));
		if (ret < 0)
			break;

		if (sysctl_get(name, value, sizeof(value)) > 0)
			fprintf(stderr, "%d) %s = %s\n", get_stack_id(), name, value);
	}
    }
    ...
    virtual const char *get_name() { return "linux2.6.26"; }
    ...
    {
	    set_stack_id(stack_id);
	    int ret = nsc_sysctl_util_set(name, value);
	    return nsc_convert_syserr_to_nscerr(ret);
    }
    ...
    {
	    set_stack_id(stack_id);
	    int ret = nsc_sysctl_util_get(name, value, len);
	    return nsc_convert_syserr_to_nscerr(ret);
    }
    ...
    {
	    set_stack_id(stack_id);
	    /* doesn't call a stack function, so don't convert retval */
...
{
	/*
	 * if we don't do this, we'd get flooded with nsc
	 * cradle code informal messages...
	 */
	if (nsc_g_l26_diagnostic < 10)
		return;
    char p[512];
    va_list ap;
...
extern "C" int printk(const char * a, ...)
{
	...
	if (nsc_g_l26_diagnostic == 0)
		return 0;

	prio = nsc_printk_get_prio(a);
	/* filter out lower priorities */
	if (nsc_g_l26_diagnostic < prio)
		return 0;

	va_start(ap, a);
	vsnprintf(p, sizeof(p), a, ap);
	va_end(ap);

	nsc_gettime(&sec, &usec);
	fprintf(stderr, "printk[%d]: %d.%06d) %s", get_stack_id(), sec, usec, p);
	return 0;
}
...
/* 
 * added support for electric fence memory debbuger
 */
extern "C" void *ef_malloc(size_t size);
extern "C" void ef_free(void *addr);
...
extern "C" void *nsc_realloc(void *mem, int size)
{
