...
if [ $# -lt 3 ];
	then
	echo "Usage  <TCP NAME e.i. CUBIC> <Processing folder i.e. radio> <total number of UEs during experiment>"
	exit 1;
fi
...
do
	grep "7.0.0.$ue"  cubic_all.raw > cubic-700$ue.dat
	grep "102.102.102.102" cubic_all.raw | grep "node $((ue+2))" > cubic_ack-700$ue.dat
done
...
for ((ue = 2; ue < $NUM_UE+2; ue++))
do
	grep "7.0.0.$ue" tcp-put.dat > tcp_send-700$ue.dat
	grep "1.0.0.2" tcp-put.dat | grep $((ULPORT+ue-1)) > tcp_ack-700$ue.dat
	grep "1.0.0.2" udp-put.dat | grep $((ULPORT+ue-1)) > udp_uplink-700$ue.dat
	grep "7.0.0.$ue" udp-put.dat > udp_downlink-700$ue.dat
done
